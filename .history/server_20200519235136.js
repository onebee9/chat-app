...
io.on('connection', socket => {
    ...
    let {username,room,id} = "";
    let userDetails={username,room,id};

    //function to provide complete user details after login
    app.post('/api/userDetails', (request, response) => {
        username = request.body.username; // this is a destructured version of const language = request.query.to, format is const{key}=request.query
