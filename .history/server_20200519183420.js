...
app.get('/api/userDetails', async (request, response) => {
    const username = request.body.language; // this is a destructured version of const language = request.query.to, format is const{key}=request.query
    const room = request.body.language;
    const id = socket.id;
};
